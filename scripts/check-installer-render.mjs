...
print_render_meta() {
	...
	printf '__META__ %s cols=%s rows=%s layout_show_logo=%s lab_width=%s render_lab_width=%s compact=%s visible=%s content_height=%s\\n' \\
		"$label" "$zero_screen_cols" "$zero_screen_rows" "$zero_screen_layout_show_logo" \\
		"$zero_screen_layout_lab_width" "$zero_screen_render_lab_width" "$zero_screen_compact" "$visible" "$content_height"
}

render_case() {
	zero_screen_title="Installing Zero"
	zero_screen_detail="Fetching the verified package."
...
screen_case() {
	...
	zero_screen "Installing Zero" "Installing Zero" "Fetching the verified package." ""
...
Finalizing npm install."
	for progress_frame in 1 24 25 48 49 200; do
		zero_animation_frame="$progress_frame"
		printf '__PROGRESS__ %s\t%s\t%s\\n' "$progress_frame" "$(zero_animation_status "Installing Zero" "$progress_details" static)" "$(zero_animation_detail "$progress_details")"
	done
}
...
const tempDir = mkdtempSync(join(tmpdir(), "zero-installer-render-"));
...
function assertInstallerProgress(progress) {
	...
	for (const [index, expectedDetail] of expectedDetails.entries()) {
		...
		check(
			progress[index].status === "Installing Zero...",
