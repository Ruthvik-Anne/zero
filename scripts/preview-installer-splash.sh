...
frames="${1:-${ZERO_SPLASH_PREVIEW_FRAMES:-300}}"
case "$frames" in
	""|*[!0-9]*)
...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/zero-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	zero_screen "Installing Zero" "Downloading Zero$(zero_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
