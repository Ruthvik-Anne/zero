...
describe("Cache Retention (ZERO_CACHE_RETENTION)", () => {
	const originalEnv = process.env.ZERO_CACHE_RETENTION;

	beforeEach(() => {
		delete process.env.ZERO_CACHE_RETENTION;
	});

	afterEach(() => {
		if (originalEnv !== undefined) {
			process.env.ZERO_CACHE_RETENTION = originalEnv;
		} else {
			delete process.env.ZERO_CACHE_RETENTION;
		}
	});
	...
	describe("Anthropic Provider", () => {
		it.skipIf(!process.env.ANTHROPIC_API_KEY)(
			"should use default cache TTL (no ttl field) when ZERO_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.ANTHROPIC_API_KEY)("should use 1h cache TTL when ZERO_CACHE_RETENTION=long", async () => {
			process.env.ZERO_CACHE_RETENTION = "long";
			const model = getModel("anthropic", "claude-haiku-4-5");
			let capturedPayload: any = null;
	...
	describe("OpenAI Responses Provider", () => {
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should not set prompt_cache_retention when ZERO_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should set prompt_cache_retention to 24h when ZERO_CACHE_RETENTION=long",
			async () => {
				process.env.ZERO_CACHE_RETENTION = "long";
				const model = getModel("openai", "gpt-4o-mini");
				let capturedPayload: any = null;
