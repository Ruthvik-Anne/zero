<p align="center">
  <picture>
    ...
    <img alt="Zero mark" src="../../assets/brand/zero-mark-black.svg" width="88">
  </picture>
</p>

<h1 align="center">Zero AI</h1>
...
```bash
npm install @zero-agent/ai
```

TypeBox exports are re-exported from `@zero-agent/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@zero-agent/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@zero-agent/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@zero-agent/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@zero-agent/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@zero-agent/ai';
...
```typescript
import { getModel, stream } from '@zero-agent/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@zero-agent/ai';
...
```typescript
import { Model, stream } from '@zero-agent/ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Zero thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@zero-agent/ai';
...
```typescript
import { getModel, complete, Context } from '@zero-agent/ai';
...
```typescript
import { Context, getModel, complete } from '@zero-agent/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@zero-agent/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@zero-agent/ai';
...
```bash
npx zero-ai login              # interactive provider selection
npx zero-ai login anthropic    # login to specific provider
npx zero-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@zero-agent/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@zero-agent/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@zero-agent/ai/oauth';
...
```typescript
import { getModel, complete } from '@zero-agent/ai';
import { getOAuthApiKey } from '@zero-agent/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@zero-agent/ai`
