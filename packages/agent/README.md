...
```bash
npm install @zero-agent/agent-core
```
...
```typescript
declare module "@zero-agent/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@zero-agent/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@zero-agent/agent-core";
