...
export const ZERO_BUILD_ID_ENV = "ZERO_BUILD_ID";
export const ZERO_LAUNCHER_PATH_ENV = "ZERO_LAUNCHER_PATH";
...
export function getDaemonRuntimeIdentity(environment: NodeJS.ProcessEnv = process.env): DaemonRuntimeIdentity {
	...
	const launcher = environment[ZERO_LAUNCHER_PATH_ENV];
	return {
		buildId: environment[ZERO_BUILD_ID_ENV] ?? bundledBuildId() ?? `release-${VERSION}`,
