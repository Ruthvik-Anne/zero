...
import { CONFIG_DIR_NAME, ENV_AGENT_DIR } from "../../config.js";
import { getProcessStartId } from "../../core/session-lease.js";
import { defaultDaemonSocketDir } from "./daemon-socket.js";

const DAEMON_SUPERVISOR_REGISTRY_DIR_ENV = "ZERO_INTERNAL_DAEMON_SUPERVISOR_REGISTRY_DIR";
...
function defaultDaemonSupervisorRegistryDir(environment: NodeJS.ProcessEnv = process.env): string {
	...
	return join(homedir(), CONFIG_DIR_NAME, "supervisor-owners");
}

/**
 * Pre-move registry location under $TMPDIR, consulted READ-ONLY while daemons
 * started before the durable-homedir move may still be running; gated off
 * whenever the registry is overridden or agent-dir-scoped.
 */
function legacyDaemonSupervisorRegistryDir(environment: NodeJS.ProcessEnv = process.env): string | undefined {
	if (environment[DAEMON_SUPERVISOR_REGISTRY_DIR_ENV] || environment[ENV_AGENT_DIR]) return undefined;
