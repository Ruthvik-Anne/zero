...
import { registerOAuthProvider, resetOAuthProviders } from "@zero-agent/ai/oauth";
...
export class ModelRegistry {
	 ...
	 *
	 * (task #27) If `config.api` is a genuinely custom (non-built-in) API — i.e.
	 * not one of the ids `resetApiProviders()` restores itself (openai-completions,
	 * anthropic-messages, etc.) — this call MUST include `config.streamSimple`,
	 * or the actual stream handler will silently stop working after any later
	 * `refresh()`/`refreshAvailableModels()` call. `refresh()` calls the vendored
	 * `resetApiProviders()`, which wipes every dynamically-registered API handler
	 * and re-registers only built-ins; the *only* thing that restores a custom
	 * handler afterward is `applyProviderConfig()` re-applying the config stored
	 * from THIS call, and it only re-registers when `streamSimple` was part of
	 * that stored config. This never affects real custom OpenAI-compatible
	 * providers (they use the built-in `"openai-completions"` api id, which
	 * always survives); it only bites a genuinely custom API implementation
	 * registered without `streamSimple` — in practice, mostly test harnesses
	 * using `registerFauxProvider()` + `registerProvider()` without re-passing
	 * `streamSimple`. Existing tests hit exactly this and work around it by
	 * passing `model` explicitly to `createAgentSession` instead of relying on
	 * settings-based auto-resolution (which calls `refreshAvailableModels()`).
	 */
