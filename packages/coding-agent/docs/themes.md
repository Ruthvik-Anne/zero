> Zero can create themes. Ask it to build one for your setup.
...
Zero loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.zero/agent/themes/*.json`
- Project: `.zero/agent/themes/*.json`
...
On first run, Zero detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.zero/agent/themes
vim ~/.zero/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, Zero reloads it automatically for immediate visual feedback.
...
Zero uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Zero falls back to the nearest approximation.
