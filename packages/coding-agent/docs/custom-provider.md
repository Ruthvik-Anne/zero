...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. Zero waits for the factory before startup continues, so the provider is available during interactive startup and to `zero model list`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map Zero levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@zero-agent/ai";
...
Credentials are persisted in `~/.zero/agent/auth.json`:
...
interface ProviderModelConfig {
  ...
  /** Maps Zero thinking levels to provider/model-specific values; null marks a level unsupported. */
