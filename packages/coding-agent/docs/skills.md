> Zero can create skills. Ask it to build one for your use case.

# Skills

Skills are self-contained capability packages that Zero loads on demand. A skill provides specialized workflows, setup instructions, helper scripts, and reference documentation for specific tasks.

Zero implements the [Agent Skills standard](https://agentskills.io/specification), warning about violations but remaining lenient. It also supports Python-backed skills: a superset of markdown skills that install Python packages into the persistent IPython kernel.
...
- [Creating Skills with Zero](#creating-skills-with-zero)
...
- Built-in: `skills/` shipped with the zero package (lowest precedence)

Discovery rules:
- In `~/.zero/agent/skills/` and `.zero/agent/skills/`, direct root `.md` files are discovered as individual skills
...
Zero ships with built-in skills that load by default:
...
For project-level Claude Code skills, add to `.zero/agent/settings.json`:
...
1. At startup, Zero scans skill locations and extracts names, descriptions, type, and file locations
...
For `web-search`, Zero exposes `web_search` in IPython. If the module defines `run()`, the module is wrapped as an async callable:
...
## Creating Skills with Zero

Zero ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:

```text
Create a project Python-backed skill named release-audit in
.zero/agent/skills/release-audit. It should expose
...
1. **Scope:** use `.zero/agent/skills/<name>/` for a project skill committed with the repository, or `~/.zero/agent/skills/<name>/` for a personal skill.
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh Zero session after adding a Python-backed skill so kernel setup can install and import the package.
...
Zero validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
