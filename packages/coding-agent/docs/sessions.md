...
Zero saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.zero/agent/sessions/`. Each session is a JSONL file with a tree structure.

```bash
zero --continue          # Continue the most recent session
zero --resume [path|id]  # Browse past sessions or resume one directly
zero --no-session        # Ephemeral mode; do not save
zero --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `zero --resume` opens the same picker at startup, and `zero --resume <path|id>` resumes a specific session.

An invalid ID exits with the closest unambiguous session ID when one is available. To open the picker and send an initial prompt after selecting a session, separate the prompt with `--`: `zero --resume -- "continue this work"`.
...
When available, Zero uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `zero --resume`.
...
When `/tree` switches away from one branch to another, Zero can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
