...
```bash
zero --mode json "Your prompt"
```

Outputs all session events as JSON lines to stdout. Useful for integrating Zero into other tools or custom UIs.
...
```bash
zero --mode json "List files" 2>/dev/null | jq -c 'select(.type == "message_end")'
```
