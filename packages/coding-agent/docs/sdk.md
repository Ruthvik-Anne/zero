> Zero can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to Zero's capabilities. Use it to embed Zero in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@zero-agent/coding-agent";
...
```bash
npm install @zero-agent/coding-agent
```
...
```typescript
import { createAgentSession } from "@zero-agent/coding-agent";
...
The `Agent` class (from `@zero-agent/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.zero/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.zero/agent/extensions/`)
- Project skills:
  - `.zero/agent/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.zero/agent/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.zero/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@zero-agent/coding-agent";
...
// Default: uses ~/.zero/agent/auth.json and ~/.zero/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@zero-agent/coding-agent";
...
- If you omit `tools`, Zero automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@zero-agent/coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.zero/agent/extensions/`, `.zero/agent/extensions/`, and `settings.json` extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@zero-agent/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@zero-agent/coding-agent";
...
1. Global: `~/.zero/agent/settings.json`
2. Project: `<cwd>/.zero/agent/settings.json`
