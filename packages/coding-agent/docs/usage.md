...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want Zero to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically as flat JSONL files under `~/.zero/agent/sessions/`. Each session header records its working directory, which the session picker uses for project-scoped views.

```bash
zero -c                  # Continue most recent session
zero -r [path|id]        # Browse sessions or resume one directly
zero --no-session        # Ephemeral mode; do not save
zero --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `zero agents`, `zero list`, or `zero attach <agent>` to find and reattach to running work. `zero stop <agent>` stops one root agent, while `zero shutdown` stops all workers and the local supervisor.
...
- `~/.zero/agent/AGENTS.md` for global instructions
...
- `.zero/agent/SYSTEM.md` for a project
- `~/.zero/agent/SYSTEM.md` globally
...
zero attach <agent>
zero stop <agent>
zero rename <agent> <name>
zero send <agent> <message>
zero schedule <list|add|cancel>
zero status
zero doctor [--fix]
zero shutdown [--force]
...
See [Zero Packages](packages.md) for package sources and security notes.
...
Use `zero session export <file> [output]` to export a session to HTML.
...
```bash
zero --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `ZERO_CODING_AGENT_DIR` | Override config directory; default is `~/.zero/agent` |
...
| `ZERO_PACKAGE_DIR` | Override package directory, useful for Nix/Guix store paths |
...
| `ZERO_CACHE_RETENTION` | Set to `long` for extended prompt cache where supported |
...
| `ZERO_KERNEL_PYTHON` | Use an existing Python environment with `ipykernel` instead of bootstrapping `~/.zero/agent/kernel-venv` |
| `VISUAL`, `EDITOR` | External editor for Ctrl+G |

The remaining `PI_*` variables are compatibility names still read by the current runtime. They do not change the application name, command, or default `~/.zero/agent` configuration path.

## Design Principles

Zero keeps the model-facing tool surface small while making the IPython runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and Zero packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

Zero preserves MIT attribution to pi-mono for its upstream lineage, but upstream Pi product claims and limitations do not describe the current Zero architecture.
