...
Sessions can be removed by deleting their `.jsonl` files under `~/.zero/agent/sessions/`.

Zero also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Zero uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@zero-agent/coding-agent/dist/` and `node_modules/@zero-agent/ai/dist/`.
...
### Base Message Types (from Zero AI)
...
### Extended Message Types (from Zero Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Zero (legacy field name)
