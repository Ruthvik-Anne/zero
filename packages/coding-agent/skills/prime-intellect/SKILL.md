...
Prime Intellect is an open superintelligence lab building open-source AGI infrastructure: a platform for RL environments, evaluations, post-training, inference, and globally distributed GPU compute. This skill lets Zero interface with Prime Intellect's products via the `prime` CLI, the default way to interface with every product below.
