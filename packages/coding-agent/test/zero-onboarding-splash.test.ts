...
import { ZeroOnboardingSplashComponent } from "../src/modes/interactive/components/zero-onboarding-splash.js";
import { initTheme } from "../src/modes/interactive/theme/theme.js";
import { ZERO_LOGO } from "../src/themes/zero-logo.js";

describe("ZeroOnboardingSplashComponent", () => {
	beforeAll(() => {
		initTheme("dark");
	});
	...
	it("renders a minimal first-run login action", () => {
		const component = new ZeroOnboardingSplashComponent(
			() => {},
			() => {},
			{ getRows: () => 36 },
		);
		...
		expect(output).toContain("Press Enter to get started");
		...
		expect(output).toContain(ZERO_LOGO.split("\n")[0].trim());
		for (const line of lines) {
			expect(visibleWidth(line)).toBeLessThanOrEqual(100);
		}
	});

	it("starts onboarding on confirm", () => {
		let selected = false;
		const component = new ZeroOnboardingSplashComponent(
			() => {
				selected = true;
	...
	it("renders a model selection action when auth is already available", () => {
		...
		expect(output).not.toContain("Press Enter to get started");
	});

	it("shows progress and ignores input while onboarding advances", () => {
		const onSelect = vi.fn();
		const onCancel = vi.fn();
		const component = new ZeroOnboardingSplashComponent(onSelect, onCancel, { getRows: () => 36 });
	...
	it("animates the splash at an interactive cadence", () => {
		...
		expect(secondRender).toContain("Welcome to Zero");
		expect(secondRender).toContain("Press Enter to get started");
	});
	...
	it("centers stacked content in narrow terminals", () => {
		...
		const logoLine = rendered.find((line) => line.includes(ZERO_LOGO.split("\n")[0].trim()));
		const brandLine = rendered.find((line) => line.includes("Welcome to Zero"));
		const hintLine = rendered.find((line) => line.includes("Press Enter to get started"));
