...
`,
	...
	it("uses ZERO_KERNEL_PYTHON as an override contract", async () => {
	...
	it("allows ZERO_KERNEL_PYTHON missing Python skill imports", async () => {
		const overridePython = join(tempDir, "override-python");
		const pythonSkill = createPythonSkill();
		writeFakePython(overridePython, ["ipykernel", "rlm", ...DEFAULT_RLM_EXTRA_IMPORT_NAMES]);
		process.env.ZERO_KERNEL_PYTHON = overridePython;
	...
	it("rejects ZERO_KERNEL_PYTHON missing default extra packages", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects ZERO_KERNEL_PYTHON with a stale rlm runtime", async () => {
		const overridePython = join(tempDir, "override-python");
